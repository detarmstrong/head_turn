...
$(document).ready(() => {
  ...
  mouseDown$
    .map(e => e.target.id)
    .filter(x => ['left', 'up', 'right', 'down'].includes(x))
    .subscribe(e => {
      // all this nesting here - russian dolls I dn't want
      Rx.Observable
        .just(e)
        .delay(300)
  ...
  // NOTE keydown event causes different codes then
  // keypress
  // HEY keydown fires repeatedly if key is held down
  //TODO only call go every 150 ms, like in the mousedown
